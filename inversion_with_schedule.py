...
def invert(ref,
    ...
    for e in range(nepochs):
        print(e,end=',')
        from elp_masking import Perturbation
        perturb = Perturbation(x,no_grad=False) 
        x_pyr = perturb.pyramid
        x_scores = model(x_pyr) #should I pass it through preprocess?
        x_feat = layer_to_invert.our_feats
        x_pyr_scores = model(x_pyr) #should I pass it through preprocess?
        x_pyr_feat = layer_to_invert.our_feats
        mse_loss = torch.nn.functional.mse_loss(ref_feat,x_feat)
        #mse_loss = torch.nn.functional.mse_loss(ref_feat,x_pyr_feat)
        pyr_loss = torch.nn_functional.mse_loss(x_pyr_feat[1:] ,x_pyr_feat[:-1])
        tv_loss = tv(x)
