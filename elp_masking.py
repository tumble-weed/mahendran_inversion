...
#         )
def create_pyramid(input,pyramid,type,max_blur,padding_mode,padding_value,num_levels):
    for sigma in torch.linspace(0, 1, num_levels):
...
class Perturbation:
    ...
    def __init__(self, input, num_levels=8, max_blur=20, type=BLUR_PERTURBATION,input_b=None,padding_mode='constant',padding_value=0,no_grad = True):
        self.type = type
        self.num_levels = num_levels
        self.pyramid = []
        assert num_levels >= 2
        ...
        if no_grad:
            with torch.no_grad():
                # dutils.pause()            
                create_pyramid(input,self.pyramid,type,max_blur,padding_mode,padding_value,num_levels):
                self.pyramid = torch.cat(self.pyramid, dim=0)
        else:
            create_pyramid(input,self.pyramid,type,max_blur,padding_mode,padding_value,num_levels):
            self.pyramid = torch.cat(self.pyramid, dim=0)
...
#class CalibratedPerturbation(Perturbation):
#    r"""Perturbation pyramid.
#
#    The class takes as input a tensor :attr:`input` and applies to it
#    perturbation of increasing strenght, storing the resulting pyramid as
...
#    """
#
#    def __init__(self, input, num_levels=8, max_blur=20, type=BLUR_PERTURBATION,input_b=None,padding_mode='constant',padding_value=0,sigmas=None,no_grad = True):
#        self.type = type
#        self.num_levels = num_levels
#        self.pyramid = []
#        assert num_levels >= 2
#        assert max_blur > 0
#        if sigmas is None:
#            sigmas_ = torch.linspace(0, 1, self.num_levels)
#            sigmas = [(1-sigma_)*max_blur for sigma_ in sigmas_]
#        if no_grad:
#            with torch.no_grad():
#                #for sigma in torch.linspace(0, 1, self.num_levels):
#                for sigma in sigmas:
